...
class LgTvController extends EventEmitter {
  ...
  async turnOn() {
    if (this.connected === true) {
      this.channelUp(); 
      this.channelDown(); //I needed 2 commands here, just firing one command would put TV in mystery mode with weird activity in logs and black screen
    }else{
        wol.wake(this.mac, {
  ...
  webOs2xStateFallback() {
    ...
    tcpp.probe(this.ip, TV_WEBSOCKET_PORT, (err, isAlive) => {
      if (this.isTvOn() && (!isAlive || !this.getForegroundAppAppId())) {
        this.webOs2xTvTurnedOff();
      } else if (!this.isTvOn() && isAlive && this.getForegroundAppAppId()) {
